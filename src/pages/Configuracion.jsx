...
const roles = ['Administrador', 'Ventas', 'Proyectos', 'Compras', 'Almacén', 'Consulta'];
...
const Configuracion = () => {
  ...
  const fetchUsers = useCallback(async () => {
    ...
    try {
      // Calling the view to get users with their emails
  ...
  const handleSaveUser = async () => {
    ...
    if (editingUser) { // Update existing user's role/name
      ...
      if (!error) {
        ...
        // The trigger will handle adding the user to public.usuarios, so we just refetch.
        setTimeout(() => fetchUsers(), 1500); // Wait a bit for the trigger to fire and replicate
        setIsDialogOpen(false);
      }
    }
    setIsSubmitting(false);
                        ...
                        <Button variant="ghost" size="icon" onClick={() => handleOpenDialog(user)} className="text-blue-500 hover:text-blue-700">
                         ...
                         <AlertDialog>
                            ...
                            <AlertDialogContent>
                                ...
                                <AlertDialogAction onClick={() => handleDeleteUser(user.id)} className="bg-red-600 hover:bg-red-700 hidden">Sí, eliminar</AlertDialogAction>
                                </AlertDialogFooter>
                            </AlertDialogContent>
                         </AlertDialog>
                      </td>
